/* === CONTROLE DO VÍDEO DE ABERTURA === */
const introModal = document.getElementById("intro-modal");
const introVideo = document.getElementById("intro-video");
const btnPularIntro = document.getElementById("pular-intro");
...
function tentarAutoplay() {
    ...
    const playVideo = () => {
        introVideo.play().then(() => {
            if (btnPularIntro) btnPularIntro.textContent = "Pular ▶️";
        }).catch(() => {
            if (btnPularIntro) btnPularIntro.textContent = "Iniciar vídeo ▶️";
        });
    };
    ...
    if (introVideo.readyState >= 3) {
        ...
        introVideo.addEventListener("canplaythrough", playVideo, { once: true });
    }
}
...
if (btnPularIntro) {
    btnPularIntro.addEventListener("click", () => {
        ...
        if (introVideo.paused && btnPularIntro.textContent.toLowerCase().includes("iniciar")) {
            introVideo.muted = false;
            introVideo.play().then(() => {
                btnPularIntro.textContent = "Pular ▶️";
            }).catch(() => {
                fecharIntro();
            });
            return;
